...
**Foundry Project**:
Azure AI Foundry 내에서 생성된 프로젝트 단위. 팀(catalog팀, image팀 등)과 1:1로 매핑된다. 각 프로젝트는 자체 **Foundry Endpoint**를 가지며, API 키와 사용량 추적의 경계가 된다.
_Avoid_: workspace (Azure ML Workspace와 혼동), hub (legacy 개념)

**Foundry Endpoint**:
Foundry Project에서 제공하는 모델 호출 엔드포인트. APIM이 백엔드로 프록시하는 대상이다. 각 Foundry Project마다 독립된 엔드포인트를 가진다.
...
**APIM Subscription**:
APIM에서 발급되는 구독 키. 하나의 Foundry Project(= APIM Product)당 **1개** 발급되며, 해당 팀원 전원이 공유한다. 프로젝트별 사용량 추적의 기본 단위이다.
...
**Azure Subscription**:
Azure 과금 및 리소스 관리의 최상위 단위. 하나의 Azure Subscription 안에 APIM Instance, Foundry Hub, 여러 Foundry Project가 생성된다.
...
- 하나의 **Azure Subscription** 안에 하나의 **APIM Instance**와 여러 **Foundry Project**가 존재한다
...
- 하나의 **APIM Subscription** 키가 Foundry Project당 1개 발급되며, 팀원이 공유한다
- end-user는 **APIM Instance** 엔드포인트 하나에 **APIM Subscription** 키만 보내면, APIM이 해당 키의 Product에 연결된 **Foundry Endpoint**로 자동 라우팅한다
- **APIM Instance**는 모든 요청을 **App Insights Telemetry**로 기록한다
- **Token Usage**는 **App Insights Telemetry**에서 추출하여 **Cost Dashboard**에 시각화된다
- **Project API Key**는 APIM 백엔드 설정에서 **Foundry Endpoint** 인증에 사용된다
